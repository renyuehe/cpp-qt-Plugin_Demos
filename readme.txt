...
一、当前路径下
二、环境变量下
三、编译时指定的路径会存在的 .exe中，直接通过 IDE的方式运行。（该方法不支持双击.exe)
...
------------------------------------- 最基础的动态库连接 -------------------------------------
-----------------------------CMake方式
#### cmake_link_dll：cmake 之  CMakeLists.txt 的方式来连接 dll动态库
...
target_link_libraries(testDll  E:/Desktop/new/libmythread.dll) #需要给出绝对路径
...
需要注意的是 MSVC 对应的 cl 编译体系， MINGW 对应的是 gcc（g++） 体系
-----------------------------
